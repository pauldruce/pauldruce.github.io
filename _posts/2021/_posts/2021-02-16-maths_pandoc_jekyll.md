...
You also keep complete creative control, and develop some skills that you can pass on to others! So let's go!
...
which, granted, if you haven't seen LaTeX before, may look daunting, but it took me ~10 seconds to type it all. And can be instantaneously rendered by the software I'm about to demonstrate.
...
But the fact that I don't have to convert my mathematical typing knowledge from one form to another removes one barrier from getting maths based content on the web.

Maths is not just about writing symbols, there are specific structures that are integral to communicating maths.
I wanted the following things:

- Theorem and proposition boxes
- Definitions
- Examples/Questions at specific points.
- Equation numbering! (why this isnt't a default of some methods is beyond me)
- Commutative diagrams and other diagrammatical means of communication.

#### Theorem and Poposition Boxes
...
The category of commutative $C^*$ algebras is equivalent to the category of Hausdorff topological spaces.
:::
:::{.proof}
...
:::{.theorem name="A famous theorem"}
An example theorem for this post:
The category of commutative $C^*$ algebras is equivalent to the
...
#### Definitions
In mathematics, a definition is a core feature, it is the building blocks that we construct theorems and proposition with. Thus, I think it's important to have them clearly labelled. Getting a numbered definition box is similar to a theorem environment above:
```
:::{.definition}
An extremely useful definition...
:::
```
...
By changing the code from ```:::{.definition}``` to ```:::{.example}``` or similar produces different coloured boxes and changes the text. This is customisable with a little dabbling in CSS^[I truly mean a little dabble. I know barely any CSS and I managed to hobble together enough to create this.]


### Github and Github pages

Github pages is an absolute gem. It really is. It provides everyone with a way to host a website that is publicly available and completely free of charge. You will need a Github account (completely free) and a little knowledge of how Git works. But this shouldn't be too hard to follow:
